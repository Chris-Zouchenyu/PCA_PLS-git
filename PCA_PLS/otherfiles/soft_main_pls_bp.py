...
train_x = []
train_y = []
for i in mat_data_key['BP50GATEST.mat'][3:]:
    if i == 'bp50_s1d_hl':
        train_x.append(mat_data['BP50GATEST.mat'][i])
    elif i == 'bp50_s1d_ll_a':
        train_x.append(mat_data['BP50GATEST.mat'][i])
    elif i == 'bp50_s1d_ll_b':
        train_x.append(mat_data['BP50GATEST.mat'][i])
    else:
        train_y.append(mat_data['BP50GATEST.mat'][i])
...
plt.plot([x for x in range(len(y_test))],y_test,marker='o', label='bp_real')
plt.plot([x for x in range(len(y_pred))],y_pred,marker='x', label='bp_predict')
plt.text(5, 5, 'R^2: {}%'.format(r2*100), fontsize=12, color='red')
plt.title('PCA/PLS, R2: {}%'.format(r2*100))
plt.legend()
plt.show()

torch.save(pls,r'parameters\bp.pth')
