...
train_x = []
train_y = []
for i in mat_data_key['CNGATEST.mat'][3:]:
    if i == 'cn_sd_hl':
        train_x.append(mat_data['CNGATEST.mat'][i])
    elif i == 'cn_sd_ll_a':
        train_x.append(mat_data['CNGATEST.mat'][i])
    elif i == 'cn_sd_ll_b':
        train_x.append(mat_data['CNGATEST.mat'][i])
    else:
        train_y.append(mat_data['CNGATEST.mat'][i])
...
plt.plot([x for x in range(len(y_test))],y_test,marker='o', label='cn_real')
plt.plot([x for x in range(len(y_pred))],y_pred,marker='x', label='cn_predict')
plt.text(5, 5, 'R^2: {}%'.format(r2*100), fontsize=12, color='red')
plt.title('PCA/PLS, R2: {}%'.format(r2*100))
plt.legend()
plt.show()

torch.save(pls,r'parameters\cn.pth')
