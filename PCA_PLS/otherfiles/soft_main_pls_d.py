...
train_x = []
train_y = []
for i in mat_data_key['D4052GATEST.mat'][3:]:
    if i == 'd_sd_hl':
        train_x.append(mat_data['D4052GATEST.mat'][i])
    elif i == 'd_sd_ll_a':
        train_x.append(mat_data['D4052GATEST.mat'][i])
    elif i == 'd_sd_ll_b':
        train_x.append(mat_data['D4052GATEST.mat'][i])
    else:
        train_y.append(mat_data['D4052GATEST.mat'][i])
...
plt.plot([x for x in range(len(y_test))],y_test,marker='o', label='d_real')
plt.plot([x for x in range(len(y_pred))],y_pred,marker='x', label='d_predict')
plt.text(5, 5, 'R^2: {}%'.format(r2*100), fontsize=12, color='red')
plt.title('PCA/PLS, R2: {}%'.format(r2*100))
plt.legend()
plt.show()

torch.save(pls,r'parameters\d.pth')
